...
public class ConnectionParser extends ConnectionBaseURL {
	public static HttpURLConnection connection;
	public static String auth;

	public ConnectionParser(String url1, String courseId ) {
		super(url1);
		this.setUrl1(url1 + courseId+"/assignments/");
	}
	public ConnectionParser(String url1, String courseId, int a ) {
		super(url1);
		this.setUrl1(url1 + courseId+"/students/");
	}

	public ConnectionParser(String url1) {
		super(url1);
	}

	public ConnectionParser(String url1, String courseId, String assignmentId) {
		this(url1, courseId);
		this.setUrl1(url1 + courseId + "/assignments/" + assignmentId + "/gradeable_students");
	}
...
// This Function Parses the Json Text File and Displays Assignments in a particular Course 
	public void displayAssignments() throws Exception {
		...
		try {
			...
			JSONArray array = (JSONArray) parser.parse(jsonData);
			Iterator i = array.iterator();
			response.append("id" + "\t\t" + "Due At" + "\t\t\t" + "Assignment Name" + "\n");
			// take each value from the json array separately
			while (i.hasNext()) {
				JSONObject innerObj = (JSONObject) i.next();
...
// This Function Parses the Json Text File and get Grades of an Assignments inside a particular Course
	...
	public static void main(String[] args) throws Exception {
		...
		f5.createNewFile();
		File f6 = new File("getassign.txt");
		f6.createNewFile();
		...
		System.out.println(listAssignments.equals("True") && submitButton.equals("True"));
		System.out.println(listStudents.equals("True") && submitButton.equals("True"));*/
		
		if(teacherTa.equals("True")){
		ConnectionParser a = new ConnectionParser(url1);
		...
		ConnectionParser a = new ConnectionParser(url1, course_id);
		URL url = new URL(a.getUrl1());
		connection = (HttpURLConnection) url.openConnection();
		connection.setRequestProperty("Authorization", "Bearer " + auth);
		connection.setRequestMethod("GET");
		...
		ConnectionParser a = new ConnectionParser(url1, course_id , 1);
		URL url = new URL(a.getUrl1());
		connection = (HttpURLConnection) url.openConnection();
		connection.setRequestProperty("Authorization", "Bearer " + auth);
		connection.setRequestMethod("GET");
		...
		ConnectionParser a = new ConnectionParser(url1, course_id , assign_id);
		URL url = new URL(a.getUrl1());
		connection = (HttpURLConnection) url.openConnection();
		connection.setRequestProperty("Authorization", "Bearer " + auth);
		connection.setRequestMethod("GET");
